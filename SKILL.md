...
name: taskflow
version: 2.2.0
description: "[v2.2.0] 长任务跨会话的存档/恢复/合并管理，自动识别意图、无需用户指定动作词。触发场景：用户描述任何要做的需求或改动（加功能/修 bug/重构）、表达中断或会话过长（先到这/明天继续）、新会话想继续之前的工作、任务完成要收尾合并、询问任务进度、要求收编/整合现有分支或 worktree（统一到 taskflow）。技能内部自动盘点项目任务状态并结合用户语义路由到：新任务建档 / checkpoint 存档 / resume 恢复 / merge 强制审查合并 / status / adopt 收编。管理：git worktree 隔离、任务拆分与子代理派发、四层索引、项目决策沉淀库、既有工作收编（anchor 挂靠）。"
---
